...
class RecipeImageHandler:
    ...
    def get_recipe_image(self, recipe_name: str, size: str = "medium") -> Optional[Dict]:
        """
        Get recipe image from Pexels API with caching
        
        Args:
            recipe_name (str): Name of the recipe
            size (str): Image size ('small', 'medium', 'large', 'original')
            
        Returns:
            Dict: Image data with URL, alt text, and attribution
        """
        # Check cache first
        cache_key = f"{recipe_name}_{size}"
        if cache_key in st.session_state.recipe_image_cache:
            return st.session_state.recipe_image_cache[cache_key]
        
        try:
            # Clean recipe name for search
            search_terms = self._generate_search_terms(recipe_name)
            
            for search_term in search_terms:
                image_data = self._search_pexels(search_term, size)
                if image_data:
                    # Cache the result
                    st.session_state.recipe_image_cache[cache_key] = image_data
                    return image_data
            
            # If no image found, return placeholder
            placeholder_data = self._get_placeholder_image(recipe_name, size)
            st.session_state.recipe_image_cache[cache_key] = placeholder_data
            return placeholder_data
            
        except Exception as e:
            logger.error(f"Error fetching image for {recipe_name}: {e}")
            return self._get_placeholder_image(recipe_name, size)
